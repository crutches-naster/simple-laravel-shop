    ...
    <div class="min-h-screen bg-gray-100 pt-20">
        ...
        <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            <div class="rounded-lg md:w-2/3">
                ...
                @if(!Cart::instance('main_cart')->content()->count())
                    Your cart is empty
                @endif

                @foreach(Cart::instance('main_cart')->content() as $product)
                    <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                        ...
                        <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                            <div class="mt-5 sm:mt-0">
                                <h2 class="text-lg font-bold text-gray-900"><a href="{{ route('products.show', $product->model ) }}">{{ $product->name }}</a></h2>
                                <p class="text-sm">${{ $product->price }}</p>
                            </div>
                            <div class="mt-4 flex flex-col items-end justify-end sm:space-y-6 sm:mt-0 sm:space-x-6">
                                <form class="flex items-center border-gray-100" action="{{ route('cart.count.update', $product->id) }}" method="POST">
                                    @csrf
