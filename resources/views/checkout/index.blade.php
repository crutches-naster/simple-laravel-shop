        ...
        <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            ...
            <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                <div class="mb-2 flex justify-between">
                    ...
                    <p class="text-gray-700">${{ Cart::instance('main_cart')->subtotal() }}</p>
                </div>
                <div class="flex justify-between">
                    <p class="text-gray-700">Tax</p>
                    <p class="text-gray-700">${{ Cart::instance('main_cart')->tax() }}</p>
                </div>
                <hr class="my-4"/>
                <div class="flex justify-between">
                    <p class="text-lg font-bold">Total</p>
                    <div class="flex flex-col items-end">
                        <p class="mb-1 text-lg font-bold">${{ Cart::instance('main_cart')->total() }}</p>
                        <p class="text-sm text-gray-700">including Tax</p>
                    </div>
                </div>
                <hr class="my-4"/>
                @include('checkout.payment_methods.paypal')
            </div>
        </div>
    </div>
</x-app-layout>
